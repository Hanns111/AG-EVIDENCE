...
def _obtener_version_ocrmypdf() -> str:
    ...
    try:
        resultado = subprocess.run(
            ["wsl", "ocrmypdf", "--version"], capture_output=True, text=True, timeout=10
        )
...
def preprocesar_pdf(
    ...
    try:
        resultado = subprocess.run(
            comando, capture_output=True, text=True, timeout=OCR_CONFIG.get("timeout_segundos", 120)
        )
        ...
        if resultado.returncode == 0:
            ...
            requirio_ocr = True
            tamanio_procesado = (
                archivo_salida.stat().st_size if archivo_salida.exists() else tamanio_original
            )
...
def preprocesar_expediente(
    ...
    for pdf in sorted(pdfs):
        resultado = preprocesar_pdf(str(pdf), directorio_salida, idioma=idioma)
