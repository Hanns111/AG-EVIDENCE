...
from config.settings import EvidenciaProbatoria, MetodoExtraccion, NivelObservacion, Observacion
...
class TDRRequirementExtractor:
    ...
    def extraer_requisitos(
        self, texto_tdr: str, nombre_archivo: str = "TDR.pdf", paginas: List[Tuple[int, str]] = None
        ...
        resultado = ResultadoExtraccionTDR(
            ...
            requisitos_perfil=sum(
                1
                for r in self.requisitos_encontrados
...
def extraer_requisitos_tdr(
    texto_tdr: str, nombre_archivo: str = "TDR.pdf", paginas: List[Tuple[int, str]] = None
) -> List[RequisitoTDR]:
    """
...
if __name__ == "__main__":
    ...
    assert not resultado2.tiene_requisito_experiencia(), (
        "NO debería detectar requisito de experiencia"
    )
