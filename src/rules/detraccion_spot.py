...
class SPOTValidator:
    ...
    def spot_aplica(
        ...
        """
        resultado = ResultadoSPOT(aplica=False, motivo="", meta={})
        ...
        if not resultado.aplica and tipo_servicio:
            ...
            if match_anexo3:
                resultado.aplica = True
                resultado.motivo = (
                    f"Servicio coincide con código {match_anexo3['codigo']} del Anexo 3 SPOT"
                )
        ...
        if monto_operacion is not None:
            ...
            if not resultado.aplica and monto_operacion >= self.monto_minimo:
                ...
                resultado.meta["advertencia"] = (
                    "Monto supera S/ 700 pero no se detectaron indicios de SPOT"
                )
    ...
    def _buscar_indicios_spot(
        self, documentos: List[DocumentoAnalizado]
    ...
    def _validar_evidencias_spot(self, documentos: List[DocumentoAnalizado]) -> List[Observacion]:
        """
...
def spot_aplica(
    documentos: List[DocumentoAnalizado], monto_operacion: float = None, tipo_servicio: str = None
) -> Tuple[bool, str, Dict]:
    """
...
def crear_documento_desde_pdf(
    nombre: str, texto: str, paginas: List[Tuple[int, str]] = None
) -> DocumentoAnalizado:
    """
    Helper para crear un DocumentoAnalizado desde datos de PDF.
    """
    return DocumentoAnalizado(nombre=nombre, texto=texto, paginas=paginas or [])
