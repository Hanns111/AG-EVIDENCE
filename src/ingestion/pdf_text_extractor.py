...
from .config import DEFAULT_THRESHOLDS, GatingThresholds
...
def _extraer_texto_ocr(
    pdf_path: Path, lang: str = "spa", dpi: int = 200, sample_pages: int = 1
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        resultado["confianza_promedio"] = (
            round(sum(confianzas) / len(confianzas), 3) if confianzas else 0.0
        )
        resultado["tiempo_ms"] = int((time.time() - inicio) * 1000)
...
def _decidir_metodo(
    direct_result: Dict[str, Any], ocr_result: Dict[str, Any], thresholds: GatingThresholds
) -> Dict[str, Any]:
    """
    ...
    """
    decision = {"metodo": "fallback_manual", "razon": "", "metodo_alternativo": None}

    # Evaluar direct_text
    direct_ok = (
        direct_result.get("error") is None
        and direct_result.get("num_chars", 0) >= thresholds.direct_text_min_chars
        and direct_result.get("num_words", 0) >= thresholds.direct_text_min_words
    )

    # Evaluar OCR
    ocr_ok = (
        ocr_result.get("error") is None
        and ocr_result.get("confianza_promedio", 0) >= thresholds.ocr_min_confidence
        and ocr_result.get("num_words", 0) >= thresholds.ocr_min_words
    )
    ...
    if direct_ok:
        ...
        if direct_result.get("error"):
            ...
            razones.append(
                f"direct_text_insuficiente: {direct_result.get('num_chars', 0)} chars < {thresholds.direct_text_min_chars}"
            )
        ...
        if ocr_result.get("error"):
            ...
            razones.append(
                f"ocr_baja_confianza: {ocr_result.get('confianza_promedio', 0):.2f} < {thresholds.ocr_min_confidence}"
            )
...
def extract_text_with_gating(
    pdf_path: Union[str, Path], lang: str = "spa", thresholds: Optional[GatingThresholds] = None
) -> Dict[str, Any]:
    """
    ...
    resultado = {
        ...
        "direct_text": {},
        "ocr": {},
        "metricas_documento": {"num_paginas": 0, "existe": False, "tamano_bytes": 0},
        "evidencia": {
            "thresholds_usados": {
    ...
    ocr_result = _extraer_texto_ocr(
        pdf_path, lang=lang, dpi=thresholds.ocr_dpi, sample_pages=thresholds.sample_pages
    )
