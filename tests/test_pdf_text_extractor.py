...
requires_pymupdf = pytest.mark.skipif(
    not FITZ_DISPONIBLE, reason="PyMuPDF (fitz) no disponible — estos tests requieren WSL2 runtime"
)
...
class TestRegla2PostRotacion:
    ...
    def test_post_rotacion_usa_validar_dimensiones(self):
        ...
        with patch.object(mod, "renderizar_pagina", return_value=mock_img), patch.object(
            mod, "preprocesar_rotacion", return_value=(mock_img_rotada, {"rotacion_grados": 0})
        ), patch.object(
            mod, "_validar_dimensiones", return_value=mock_img_validada
        ) as mock_val, patch.object(
            mod, "ejecutar_ocr", return_value=mock_ocr_result
        ), patch.object(mod, "verificar_ocr", return_value=(True, "ok", "paddleocr")), patch.object(
            mod, "fitz"
        ) as mock_fitz:
            mock_doc = MagicMock()
