...
from src.tools.ocr_preprocessor import _ruta_windows_a_wsl, preprocesar_expediente, preprocesar_pdf
...
def test_pdf_nativo_no_modifica():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("src.tools.ocr_preprocessor.subprocess.run") as mock_run:
            ...
            resultado = preprocesar_pdf(str(pdf_path), str(salida_dir), idioma="spa")
        ...
        with patch("src.tools.ocr_preprocessor.subprocess.run") as mock_run:
            ...
            try:
                resultados = preprocesar_expediente(
                    str(carpeta_expediente), str(salida_dir), idioma="spa"
                )
...
def test_trazabilidad_completa():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("src.tools.ocr_preprocessor.subprocess.run") as mock_run:
            ...
            with patch(
                "src.tools.ocr_preprocessor._obtener_version_ocrmypdf", return_value="17.1.0"
            ):
                resultado = preprocesar_pdf(str(pdf_path), str(salida_dir), idioma="spa")
